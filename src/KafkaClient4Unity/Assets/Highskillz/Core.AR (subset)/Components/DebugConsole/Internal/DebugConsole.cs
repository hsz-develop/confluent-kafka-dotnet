...
namespace Highskillz.Core.AR
{
    ...
    public class DebugConsole : MonoBehaviour
    {
        ...
        void HandleLog(string logString, string stackTrace, LogType type)
        {
            ...
            {
                DebugConsole.LogWithTime(logString, DebugConsoleLevel.Error);
            }
            if(type == LogType.Warning)
            {
                DebugConsole.LogWithTime(logString, DebugConsoleLevel.Warning);
            }
            if(type == LogType.Log)
            {
                DebugConsole.LogWithTime(logString, DebugConsoleLevel.Debug);
            }
        }
    }
}